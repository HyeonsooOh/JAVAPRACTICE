...
public class JqAjaxController2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		// 버전 2) ajax 로 실제 값을 여러개 보내고 싶을때 => 정석
		// => JSON (JavaScript Object Notation : 자바스크립트 객체 표기법)
		// ajax 통신 시 데이터 전송에 이용되는 포맷 형식중 하나
		
		/*
		 * JSON 처리 시 사용되는 클래스 종류
		 * => 자바에서 기본적으로 제공 X (라이브러리 필요 .jar)
		 * https://code.google.com/archive/p/json-simple/downloads 접속 후 다운로드
		 * json-simple-1.1.1.jar 다운로드 후 WEB-INF\lib 에 붙여넣기
		 * 
		 * 1. JSONArray[값1, 값2, ...] => 배열 형태로 넘기는거 가능
		 * 2. JSONObject{키1:밸류1, 키2:밸류2, ..} => 객체 형태로 넘기는거 가능
		 */
		...
		// => 응답할 데이터의 컨텐트타입을 제대로 지정해야 문자열 형식으로 넘어가지 않음
		// => 만약에 text/html 로 보내게 되면 문자열로써 "["홍길동", 30]" 이 전달됨
		
		// 보내기
		response.getWriter().print(jArr);
		*/
		
		// 마찬가지로 JSONObject 로도 넘기는거 가능
		JSONObject jObj = new JSONObject(); // {} 빈 객체(자바스크립트) 이 만들어짐
		// 객체에 값 담기 => put 메소드
		jObj.put("name", name); // { name:"홍길동" }
		jObj.put("age", age); // { name:"홍길동", age:30 }
		
		// json 타입의 값이라고 컨텐트 타입, 인코딩  지정
		response.setContentType("application/json; charset=UTF-8");
